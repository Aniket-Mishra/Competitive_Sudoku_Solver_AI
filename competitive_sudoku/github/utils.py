...
def is_illegal(*, move: Move, state: GameState) -> bool:
    ...
    if state.board.get((move.square[0], move.square[1])) != SudokuBoard.empty:
        return True
    ...
    if any(
            state.board.get((row, move.square[1])) == move.value
    ...
    if any(
            state.board.get((move.square[0], col)) == move.value
    ...
    return any(
        ...
        for row, col in block_range(row=move.square[0], col=move.square[1], board=state.board)
    )
...
def calculate_move_score(game_state: GameState, move: Move) -> int:
    ...
    for col in range(game_state.board.board_width()):
        if game_state.board.get((move.square[0], col)) == SudokuBoard.empty:
            row_complete = False
            break

    # Check if completed a column
    for row in range(game_state.board.board_height()):
        if game_state.board.get((row, move.square[1])) == SudokuBoard.empty:
            col_complete = False
            break
