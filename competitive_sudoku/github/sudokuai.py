...
class SudokuAI(SudokuAI):
    ...
    def update_legal(self, game_state: GameState, move: Move, moves: dict) -> list:
        ...
        for row in range(game_state.board.board_height()):
            ...
            if moves["legal"][row, move.square[1], move.value]:
                _moves_invalidated.append(Move((row, move.square[1]), move.value))
                moves["legal"][row, move.square[1], move.value] = False  # Set legal status
        for column in range(game_state.board.board_width()):
            if moves["legal"][move.square[0], column, move.value]:
                _moves_invalidated.append(Move((move.square[0], column), move.value))
                moves["legal"][move.square[0], column, move.value] = False
        for row, col in block_range(row=move.square[0], col=move.square[1], board=game_state.board):
    ...
    def minimax(
        ...
        """
        block_indices = block_index(move.square[0], move.square[1], game_state.board)

        # Apply move, update resulting scores
        # Update legal moves and count newly invalidated moves
        game_state.board.put((move.square[0], move.square[1]), move.value)
        ...
        moves["free"]["row"][move.square[0]] -= 1
        moves["free"]["col"][move.square[1]] -= 1
        ...
        if depth == 0 or moves["count"] == 0:
            ...
            if maximizing_player == current_player:  # maximising player
                ...
                for try_move in moves["initial"]:
                    if not moves["legal"][try_move.square[0], try_move.square[1], try_move.value]:
                        continue
        ...
        for inv_move in _moves_invalidated:
            moves["legal"][inv_move.square[0], inv_move.square[1], inv_move.value] = True
        game_state.scores[current_player] -= _score_achieved
        game_state.board.put((move.square[0], move.square[1]), 0)
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        for move in moves["initial"]:
            moves["legal"][move.square[0], move.square[1], move.value] = True
        ...
        moves["free"] = {
            "row": [
                len(set(((m.square[0], m.square[1]) for m in moves["initial"] if m.square[0] == y)))
                for y in range(board_size)
            ],
            "col": [
                len(set(((m.square[0], m.square[1]) for m in moves["initial"] if m.square[1] == x)))
            ...
            "block": [
                [
                    len(
                        set(
                            (
                                (m.square[0], m.square[1])
                                for m in moves["initial"]
                                if block_index(m.square[0], m.square[1], game_state.board) == (i, j)
                            )
                        )
                    )
