...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def trapping_strategy(self, game_state: GameState, ai_player_index: int, depth: int):
        if depth == 0 or self.is_terminal(game_state):
            return self.calculate_opponent_moves(game_state, ai_player_index)

        valid_moves = self.get_valid_moves(game_state)

        least_opponent_moves = self.calculate_opponent_moves(
            game_state, ai_player_index)

        for move in valid_moves:
            next_state = self.simulate_move(game_state, move)

            opponent_moves = self.calculate_opponent_moves(
                next_state, ai_player_index)

            if opponent_moves < least_opponent_moves:
                least_opponent_moves = opponent_moves
        ...
        for depth in range(1, depth + 1):
            ...
            for move in valid_moves:
                ...
                if state == "trapped" or "minimax":
                    ...
                    trapping_score = self.trapping_strategy(
                        next_state, ai_player_index)
