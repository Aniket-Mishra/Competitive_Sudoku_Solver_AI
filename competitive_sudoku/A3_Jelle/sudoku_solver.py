...
class SudokuSolver:
    def __init__(self, board, N, m, n):
        ...
        self.board = copy.deepcopy(board)  # Make a copy of the board
        ...
        # The board is always made up of N blocks (e.g. (N/m) * (N/n) = n * m = N)
        self.blocks = {block_index: set() for block_index in range(N)}
        self._initialize()
    ...
    def solve(self):
        ...
        if not cell:
            ...
            # return True if all cells are non-zero (sudoku is solved) otherwise return False (sudoku is unsolvable)
    ...
    def get_board_as_dict(self):
        ...
        if not self.solve():
            raise ValueError(
                "Sudoku no longer solvable (should be notified by the Oracle, so in practice does not occur)")
