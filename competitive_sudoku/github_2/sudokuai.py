...
from github_2.helpers import game_helpers, heuristics, taboo_move_calculation
...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    class Node:
        ...
        def extend_node(self):
            ...
            if len(self.children) > 0:
                raise Exception(
                    "extend tree should not be called on an already extended node!")

            valuable_moves, taboo_moves, can_score = SudokuAI.get_valuable_moves(
                self.game_state)
            ...
            if taboo_moves and self._want_to_play_taboo(can_score):
                ...
                for move in taboo_moves:
                    new_game_state = game_helpers.simulate_move(
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        while i < max_depth:
            ...
            value, optimal_move = self.minimax(
                root, i, maximizing_player, -100000, 100000)
    ...
    def get_valuable_moves(game_state: GameState) -> Tuple[List[Move], List[Move], bool]:
        ...
        (legal_moves, rows, columns,
         blocks) = game_helpers.compute_all_legal_moves(game_state)
        ...
        taboo_move_calculation.obvious_singles(
            game_state, legal_moves, taboo_moves)
        taboo_move_calculation.hidden_singles(
            game_state, legal_moves, taboo_moves)

        taboo_move_calculation.locked_candidates_rows(
            game_state, legal_moves, rows, blocks, taboo_moves)
        taboo_move_calculation.locked_candidates_columns(
            game_state, legal_moves, columns, blocks, taboo_moves)
        ...
        legal_moves = heuristics.force_highest_points_moves(
            game_state, legal_moves, rows, columns, blocks)
